...
export const findUserByEmail = async (email) => prisma.user.findUnique({ where: { email, deleted_at: null }, include: { role: true } });
export const findUserById = async (id) => prisma.user.findUnique({ where: { id, deleted_at: null }, include: { role: true } });
