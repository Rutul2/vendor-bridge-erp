...
export const findInvoices = async ({ skip, take, status, vendor_id }) => {
  ...
  return prisma.invoice.findMany({
    ...
    include: { vendor: true, purchase_order: { include: { items: true } } },
  });
};

export const countInvoices = async ({ status, vendor_id }) => prisma.invoice.count({ where: { AND: [status ? { status } : {}, vendor_id ? { vendor_id } : {}] } });

export const findInvoiceById = async (id) => prisma.invoice.findUnique({ 
  where: { id },
  include: { purchase_order: { include: { items: true } }, vendor: true }
});
